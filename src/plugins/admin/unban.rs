...
lazy_static! {
    static ref LOGGER: std::sync::Arc<LoggerHandle> = {
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(get_logger("UnbanChatPlugin"))
        })
    };
}
#[TeloxidePlugin(commands = ["unban", "unban@KissShotChkBot"])]
pub struct UnbanChatPlugin;
impl UnbanChatPlugin {
    async fn handle(&self, bot: &Bot, message: &Message, msg: &str) {
        self.unban_chat(bot, message, msg).await;
    }
    async fn unban_chat(&self, bot: &Bot, message: &Message, msg: &str) {
        ...
        if !check_banned(target_id).await {
            let _ = bot
                .edit_message_text(
                    wait_msg.chat.id,
                    wait_msg.id,
                    &format!(
                        "<b>Unban Failed ❌</b>\n\n\
                         <b>Reason:</b> This user is not banned.\n\
                         <b>Timestamp:</b> {}",
                        timestamp
                    ),
                )
        ...
        if sql::update_user(target_id, &update_user).await.is_ok() {
            ...
            let _ = bot
                ...
                .await;
            LOGGER
                .info(&format!("User unbanned successfully: {}", target_id))
                .await;
        } else {
            LOGGER
                .error(&format!("Failed to unban user: {}", target_id))
                .await;
            let _ = bot
                .edit_message_text(
                    wait_msg.chat.id,
