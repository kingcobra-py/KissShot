...
lazy_static! {
    static ref LOGGER: std::sync::Arc<LoggerHandle> = {
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(get_logger("BanChatPlugin"))
        })
    };
}
#[TeloxidePlugin(commands = ["ban", "ban@KissShotChkBot"])]
pub struct BanChatPlugin;
impl BanChatPlugin {
    async fn handle(&self, bot: &Bot, message: &Message, msg: &str) {
        self.ban_chat(bot, message, msg).await;
    }
    async fn ban_chat(&self, bot: &Bot, message: &Message, msg: &str) {
        ...
        if check_banned(target_id).await {
            let _ = bot
                .edit_message_text(
                    ...
                    &format!(
                        "<b>Ban Failed ❌</b>\n\n\
                         <b>Reason:</b> This user is already banned.\n\
                         <b>Timestamp:</b> {}",
                        timestamp
                    ),
                )
        ...
        if sql::update_user(target_id, &update_user).await.is_ok() {
            ...
            let _ = bot
                ...
                .await;
            LOGGER
                .info(&format!("User banned successfully: {}", target_id))
                .await;
        } else {
            LOGGER
                .error(&format!("Failed to ban user: {}", target_id))
                .await;
            let _ = bot
                .edit_message_text(
                    wait_msg.chat.id,
