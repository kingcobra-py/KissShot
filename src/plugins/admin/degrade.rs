...
lazy_static! {
    static ref LOGGER: std::sync::Arc<LoggerHandle> = {
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(get_logger("DegradeChatPlugin"))
        })
    };
}
#[TeloxidePlugin(commands = ["degrade", "degrade@KissShotChkBot"])]
pub struct DegradeChatPlugin;
impl DegradeChatPlugin {
    async fn handle(&self, bot: &Bot, message: &Message, msg: &str) {
        self.degrade_chat(bot, message, msg).await;
    }
    async fn degrade_chat(&self, bot: &Bot, message: &Message, msg: &str) {
        ...
        if check_banned(user_id).await {
            let _ = bot
                .edit_message_text(
                    ...
                    &format!(
                        "<b>Degradation Failed ❌</b>\n\n\
                         <b>Reason:</b> Banned users cannot be degraded.\n\
                         <b>Timestamp:</b> {}",
                        timestamp
                    ),
                )
        ...
        if sql::update_user(target_id, &update_user).await.is_ok() {
            ...
            let _ = bot
                ...
                .await;
            LOGGER
                .info(&format!("User degraded successfully: {}", target_id))
                .await;
        } else {
            LOGGER
                .error(&format!("Failed to degrade user: {}", target_id))
                .await;
            let _ = bot
                .edit_message_text(
                    wait_msg.chat.id,
